...
int
ReadFASTA(FASTAFILE *ffp, char **ret_seq, char **ret_name, size_t *ret_L)
{
  char *s;
  char *name;
  char *seq;
  ...
  while (fgets(ffp->buffer, FASTA_MAXLINE, ffp->fp))
	...
	{
	  // jumps spaces, newlines  and tabs
    if ( isspace(*s)) {
      continue;
    }
	  if (! isalnum(*s) && (*s) != '-') {
      fprintf(stderr, "Input file has a character that is not alphanumeric, nor a gap. Aborting.");
      exit(-1);
    }
